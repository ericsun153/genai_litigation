...
#
# Purpose:
#   Run NLP analysis on LexisNexis + CourtListener enriched cases:
#   1. final filtering
#   2. doctrine scoring
#   3. topic modeling
#   4. clustering
...
def safe_bar_plot(series, title, xlabel, ylabel, path, figsize=(10, 5)):
...
if len(core_df) == 0:
    raise ValueError("No cases left after NLP filter. Check previous pipeline.")
...
def classify_case_theme(row):
    ...
    if any(t in combined for t in ["fair use", "transformative"]):
        return "fair_use_dispute"

    if any(t in combined for t in ["registration", "human authorship", "copyright office"]):
        return "ai_authorship_registration"

    return "other_ai_copyright"
...
for topic_idx, topic in enumerate(nmf.components_):
    ...
    topic_labels[topic_idx] = ", ".join(top_words[:7])
    print(f"Topic {topic_idx}: {', '.join(top_words)}")

core_df["dominant_topic"] = topic_matrix.argmax(axis=1)
core_df["dominant_topic_score"] = topic_matrix.max(axis=1)
core_df["topic_keywords"] = core_df["dominant_topic"].map(topic_labels)


# ==========================================================
# 7. PER-CASE TF-IDF KEYWORDS
# ==========================================================
...
for cluster_id in range(N_CLUSTERS):
    ...
    cluster_labels[cluster_id] = ", ".join(top_words[:7])
...
core_df = pd.concat(
    [core_df.reset_index(drop=True), nearest_df],
    axis=1
)
...
# Topic distribution
safe_bar_plot(
    core_df["dominant_topic"].value_counts().sort_index(),
    "NLP Topic Distribution",
    "Topic",
    "Number of Cases",
    f"{FIG_DIR}/07_topic_distribution.png",
)
